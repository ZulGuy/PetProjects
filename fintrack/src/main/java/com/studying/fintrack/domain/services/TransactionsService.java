...
import com.studying.fintrack.domain.utils.AuthoritiesUtil;
...
public class TransactionsService {
  ...
  /**
   * Це основа, потрібно усі сервісні методи зробити по аналогії цього, так як тут вже прописано 2
   * сценарії в залежності від ролі юзера. Також треба подивитись наскільки сек'юрно написаний цей
   * метод.
   *
   * @return
   */
  ...
  public Transaction updateTransaction(int id, Transaction transaction) {
    User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
    Transaction updatedTransaction = AuthoritiesUtil.isAdmin(user)
        ? transactionsRepository.findById(id).orElseThrow(
        () -> new EntityNotFoundException("Transaction not found in DB!")
    )
        : transactionsRepository.findByIdAndUserId(id, user.getId())
            .orElseThrow(
                () -> new EntityNotFoundException("Transaction not found in DB!")
            );
  ...
  public List<Transaction> searchTransactions(TransactionSearchFilter filter) {
    ...
    if (filter.from() != null && filter.to() != null) {
      spec = spec.and(
          TransactionSpecification.betweenDates(Timestamp.valueOf(filter.from()),
              Timestamp.valueOf(filter.to())));
    }
